...
**Status:** 456 tests, all passing (migrations through 0048) — deterministic
...
Total suite count: **456 tests, all passing** (see the status line at the top).
`test_zz_payroll.py` (20) — payroll finalization (P4-4): freeze the computed
...
survive the summary; CSV export shape + 404. **Payment batches (0048):** create
...
back to unpaid (stays finalized), lifecycle audits, 404s. **Assignment-audit
CSV:** tournament-wide trail export shape (created/finalized/paid present) + 404.
