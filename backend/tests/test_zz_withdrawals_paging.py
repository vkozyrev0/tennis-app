"""Server-side withdrawals search + paging (q / limit / offset + X-Total-Count)."""
import uuid
...
@pytest.fixture()
def trio():
    tag = "Wpage" + str(uuid.uuid4().int)[:6]
    t = _ok(client.post("/api/tournaments", json={
        "name": "W " + tag, "type": "junior",
    ...
    for first in ["Alice", "Bob", "Cara"]:
        ...
        row = _ok(client.post(f"/api/tournaments/{t['id']}/withdrawals", json={
            "usta_number": usta, "first_name": first, "last_name": tag,
            "reason": "injury",
        }))
        ids.append(row["id"])
...
def test_q_filters_and_counts(trio):
    tid, tag, ids = trio
    r = client.get(f"/api/tournaments/{tid}/withdrawals?q={tag}")
    rows = _ok(r, 200)
...
def test_limit_offset_page_disjoint(trio):
    tid, tag, ids = trio
    r1 = client.get(f"/api/tournaments/{tid}/withdrawals?q={tag}&limit=2")
    page1 = _ok(r1, 200)
    assert len(page1) == 2
    assert r1.headers["X-Total-Count"] == "3"
    page2 = _ok(client.get(f"/api/tournaments/{tid}/withdrawals?q={tag}&limit=2&offset=2"), 200)
    assert len(page2) == 1
...
def test_no_params_returns_all(trio):
    tid, tag, ids = trio
    r = client.get(f"/api/tournaments/{tid}/withdrawals")
    rows = _ok(r, 200)
