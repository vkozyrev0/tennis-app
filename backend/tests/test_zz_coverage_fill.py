...
def _fill(tid, oid, date, role, site_id=None):
    body = {"official_id": oid, "work_date": date, "working_as": role}
    if site_id is not None:
        body["site_id"] = site_id
    return client.post(f"/api/tournaments/{tid}/coverage-fill", json=body)
...
def test_fill_creates_assignment_and_day():
    ...
    s = _ok(_fill(t["id"], o["id"], DAY, "chair_umpire", site["id"]))
...
def test_fill_rejects_uncertified_role():
    ...
    assert _fill(t["id"], o["id"], DAY, "chair_umpire", site["id"]).status_code == 409
...
def test_busy_elsewhere_flag():
    ...
    site = attach_venue_site(client, _ok, t1["id"])
    o = _official("chair_umpire")
    _ok(_fill(t1["id"], o["id"], DAY, "chair_umpire", site["id"]))  # now busy DAY in t1
