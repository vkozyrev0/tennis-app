...
from ..models import AdminUserCreate, AdminUserOut, PasswordReset
from ..security import get_current_user, hash_pw

router = APIRouter(prefix="/api/admin/users", tags=["admin-users"])

_COLS = "id, username, role, created_at"
...
def create_admin(body: AdminUserCreate, conn=Depends(db_dep)):
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"INSERT INTO user_account (username, password_hash, role) "
                f"VALUES (%s, %s, 'admin') RETURNING {_COLS}",
                (body.username, hash_pw(body.password)),
            )
            return cur.fetchone()
